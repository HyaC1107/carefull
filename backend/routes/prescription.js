...
const PRESCRIPTION_FEATURE_ENABLED = false;

const send_prescription_disabled = (res) => (
    sendError(res, 503, 'Prescription OCR feature is not ready yet.')
);
...
const prescription_upload_middleware = PRESCRIPTION_FEATURE_ENABLED
...
router.post('/preview', verifyToken, prescription_upload_middleware, async (req, res) => {
    if (!PRESCRIPTION_FEATURE_ENABLED) {
        return send_prescription_disabled(res);
    }
